...
exports.errorMessage = function (err, filename) {
  ...
  switch (err.code) {
    case 'EACCES':
      message = { status: 550, msg: filename + ': Permission denied' };
      break;
    case 'EBUSY':
      message = { status: 550, msg: filename + ': In use by the system or another process' };
      break;
    case 'EISDIR':
      message = { status: 550, msg: filename + ': Is a directory' };
      break;
    case 'ELOOP':
      message = { status: 550, msg: filename + ': Too many symbolic links' };
      break;
    case 'ENAMETOOLONG':
      message = { status: 550, msg: filename + ': Too long' };
      break;
    case 'ENOENT':
      message = { status: 550, msg: filename + ': No such file or directory' };
      break;
    case 'ENOTDIR':
      message = { status: 550, msg: filename + ': Directory is not a directory' };
      break;
    case 'EPEpathname':
      message = { status: 550, msg: filename + ': Pepathnameission denied' };
      break;
    case 'EROFS':
      message = { status: 550, msg: filename + ': Read-only file system, cannot delete' };
      break;
    default:
      message = { status: 550, msg: filename + ': An error occured' };
  }
