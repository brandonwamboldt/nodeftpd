...
/**
 * FTP encode a pathname.
 */
exports.encodePathname = function (pathname) {
...
exports.can = function (mode, stat, user) {
  var perm_bits = {
    r: { user : 400, group: 40, world: 4 },
    w: { user : 200, group: 20, world: 2 },
    x: { user : 100, group: 10, world: 1 }
  };
  ...
  if (user.uid === stat.uid && stat.mode & perm_bits[mode].user) {
    ...
    return true;
  } else if (stat.mode & perm_bits[mode].world) {
    return true;
  } else {
    return false;
  }
...
var resolveAndPassThruOnePathArg = function (func) {
  return function () {
    ...
    args[0] = exports.resolve(args[0]);
    return fs[func].apply(fs[func], args);
  };
};
