...
exports.isDaemon = function () {
  return (process.argv.length >= 3 && process.argv[2].match(/--daemon/));
};
...
exports.killAllWorkers = function () {
  ...
  for (var i = 0; i < supervisor.pool.length; i++) {
    supervisor.pool[i].kill('SIGTERM');
  }
};
...
exports.spawnWorker = function () {
  ...
  var worker = fork('./bin/worker', args);
